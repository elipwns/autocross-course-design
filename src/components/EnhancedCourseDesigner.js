...
import CourseDesignGuide from './CourseDesignGuide';
import CourseElementsPanel from './CourseElementsPanel';
import '../styles/CourseDesignEnhanced.css';

/**
 * EnhancedCourseDesigner component - Improved UI for course design workflow
 * With integrated canvas functionality for drawing courses
 */
function EnhancedCourseDesigner({ venue, onCourseComplete }) {
  const canvasRef = useRef(null);
  const containerRef = useRef(null);
  const [drawingMode, setDrawingMode] = useState('start'); // 'start', 'finish', 'element', 'course'
  ...
  const isCourseComplete = startPoint && finishPoint && courseLines.length > 0;
  ...
  // Draw a cone
  const drawCone = useCallback((ctx, position, isSelected, offsetX, offsetY, currentZoom) => {
  ...
  const drawSlalom = useCallback((ctx, positions, isSelected, offsetX, offsetY, currentZoom) => {
    ...
    if (e.button === 0) {
      ...
      switch (drawingMode) {
        ...
        case 'start':
          ...
          // Auto-switch to finish point tool after placing start point
          setDrawingMode('finish');
          break;
        case 'finish':
          setFinishPoint(imageCoords);
          // Auto-switch to element tool after placing finish point
          setDrawingMode('element');
          setSelectedElement('cone'); // Default element
          break;
        case 'element':
  ...
  const handleMouseUp = (e) => {
    ...
    // Don't stop drawing on mouse up for course lines
    // But do stop drawing for other tools
    if (drawingMode !== 'course') {
      setIsDrawing(false);
    }
  };
  
  // Handle click to complete a line
  const handleClick = (e) => {
    // Only handle clicks when in course drawing mode and already drawing
    if (drawingMode === 'course' && isDrawing && currentLine.length > 1) {
  ...
  const handleKeyDown = (e) => {
    // Enter key to complete line
    if (e.key === 'Enter' && isDrawing && currentLine.length > 1) {
      completeCourseDrawing();
    }
    
    // Escape key to cancel line
  ...
  const handleToolSelect = (tool) => {
    // If switching away from course drawing, complete the current line
    if (drawingMode === 'course' && tool !== 'course' && isDrawing) {
      completeCourseDrawing();
    }
  ...
  const handleElementSelect = (elementType) => {
    // If switching away from course drawing, complete the current line
    if (drawingMode === 'course' && isDrawing) {
      completeCourseDrawing();
    }
  ...
  const addCone = (position) => {
    ...
    // No longer restoring previous zoom and offset to maintain zoom level
  };
  ...
  const clearCourse = () => {
    // Using window.confirm instead of just confirm to avoid ESLint error
    if (window.confirm('Are you sure you want to clear the entire course?')) {
      setCourseLines([]);
      setCurrentLine([]);
      setStartPoint(null);
      setFinishPoint(null);
  ...
  // Clear only course lines
  const clearLines = () => {
    if (window.confirm('Are you sure you want to clear all course lines?')) {
      setCourseLines([]);
      setCurrentLine([]);
      setIsDrawing(false);
    }
  ...
  // Clear only course elements (cones, etc.)
  const clearElements = () => {
    if (window.confirm('Are you sure you want to clear all course elements?')) {
      setElements([]);
    }
  };
  ...
  const handleFinishCourse = () => {
    if (!isCourseComplete) {
      alert('Please add a start point, finish point, and at least one course line.');
      return;
    }
  ...
  return (
    <div className="course-designer-enhanced" tabIndex="0" onKeyDown={handleKeyDown}>
      <div className="course-designer-sidebar left">
        <CourseElementsPanel 
      ...
      <div className="course-designer-main">
        <div className="drawing-tools-enhanced">
          <button 
            className={`tool-button-enhanced ${drawingMode === 'start' ? 'active' : ''}`}
            onClick={() => handleToolSelect('start')}
          >
            <div className="tool-icon">🏁</div>
            <div className="tool-label">Start Point</div>
          </button>
          
          <button 
            className={`tool-button-enhanced ${drawingMode === 'finish' ? 'active' : ''}`}
            onClick={() => handleToolSelect('finish')}
          >
            <div className="tool-icon">🏁</div>
            <div className="tool-label">Finish Point</div>
          </button>
          
          <button 
            className={`tool-button-enhanced ${drawingMode === 'element' ? 'active' : ''}`}
            onClick={() => handleToolSelect('element')}
          >
            <div className="tool-icon">🔶</div>
            <div className="tool-label">Elements</div>
          </button>
          
          <button 
            className={`tool-button-enhanced ${drawingMode === 'course' ? 'active' : ''}`}
            onClick={() => handleToolSelect('course')}
          >
            <div className="tool-icon">✏️</div>
            <div className="tool-label">Course Line</div>
          </button>
          
          <button 
            className="tool-button-enhanced"
            onClick={completeCourseDrawing}
            disabled={!isDrawing || currentLine.length < 2}
          >
            <div className="tool-icon">✓</div>
            <div className="tool-label">Complete Line</div>
          </button>
          ...
          >
            <div className="tool-icon">🗑️</div>
            <div className="tool-label">Clear All</div>
          </button>
        </div>
        ...
        <div className="canvas-container" ref={containerRef}>
          ...
          {isDrawing && (
            <div className="drawing-instructions">
              Double-click to finish line (or press Enter)
            </div>
          )}
        </div>
        ...
        <div className="course-actions">
          ...
          >
            {isCourseComplete ? 'Save Course & Continue' : 'Complete Required Elements'}
          </button>
        </div>
      </div>
      
      <div className="course-designer-sidebar right">
        <CourseDesignGuide 
          courseLines={courseLines}
          startPoint={startPoint}
          finishPoint={finishPoint}
          currentLine={currentLine}
          onCompleteLineClick={completeCourseDrawing}
...
export default EnhancedCourseDesigner;